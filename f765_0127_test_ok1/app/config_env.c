...
NETIF_PAR flash_ip_par;
NETIF_REMOTE_PAR flash_remoteip_par;


//ADDR_FLASH_SECTOR_1
#define ENV_START_ADDR     ADDR_FLASH_SECTOR_7 //ADDR_FLASH_SECTOR_11   
...
#define ENV_END_ADDR     ((uint32_t)0x08100000-1)   //((uint32_t)0x08200000-1)
...
static void config_data_init(void)
{
    ...
    memcpy(&flash_ip_par.mac[0], &UserReg.Dat.mac[0], 6);
    memcpy(&flash_ip_par.ip[0], &UserReg.Dat.srcIp[0], 4);
    memcpy(&flash_ip_par.gateway[0], &UserReg.Dat.gateway[0], 4);
    memcpy(&flash_ip_par.netmask[0],  &UserReg.Dat.netmask[0], 4);
    flash_ip_par.port = UserReg.Dat.rxPort;
    memcpy(&flash_remoteip_par.ip[0], &UserReg.Dat.remoteIp[0], 4);
    flash_remoteip_par.port = UserReg.Dat.txPort;
...
{
    flash_ip_par.ip[0] = Get_IP_Fragment(0);//FACTORY_IP_0;
    flash_ip_par.ip[1] = Get_IP_Fragment(1);//FACTORY_IP_1;
    flash_ip_par.ip[2] = Get_IP_Fragment(2);//FACTORY_IP_2;
    flash_ip_par.ip[3] = Get_IP_Fragment(3);//FACTORY_IP_3;

    flash_ip_par.netmask[0] = FACTORY_NETMASK_0;
    flash_ip_par.netmask[1] = FACTORY_NETMASK_1;
    flash_ip_par.netmask[2] = FACTORY_NETMASK_2;
    flash_ip_par.netmask[3] = FACTORY_NETMASK_3;

    flash_ip_par.gateway[0] = FACTORY_GATEWAY_0;
    flash_ip_par.gateway[1] = FACTORY_GATEWAY_1;
    flash_ip_par.gateway[2] = FACTORY_GATEWAY_2;
    flash_ip_par.gateway[3] = FACTORY_GATEWAY_3;

    flash_ip_par.mac[0] = Get_MAC_Fragment(0);  //FACTORY_MAC_B0;
    flash_ip_par.mac[1] = Get_MAC_Fragment(1);  //FACTORY_MAC_B1;
    flash_ip_par.mac[2] = Get_MAC_Fragment(2);  //FACTORY_MAC_B2;
    flash_ip_par.mac[3] = Get_MAC_Fragment(3);  //FACTORY_MAC_B3;
    flash_ip_par.mac[4] = Get_MAC_Fragment(4);  //FACTORY_MAC_B4;
    flash_ip_par.mac[5] = Get_MAC_Fragment(5);  //FACTORY_MAC_B5;

    flash_ip_par.port = FACTORY_UDP_IN_PORT;
}


static void load_Factory_remote_ipPar(void)
{
    flash_remoteip_par.ip[0] = FACTORY_REMOTE_IP_0;     //192
    flash_remoteip_par.ip[1] = FACTORY_REMOTE_IP_1;      //168
    flash_remoteip_par.ip[2] = FACTORY_REMOTE_IP_2;       //0
    flash_remoteip_par.ip[3] = FACTORY_REMOTE_IP_3;     //190
    flash_remoteip_par.port = FACTORY_UDP_OUT_PORT;
}
