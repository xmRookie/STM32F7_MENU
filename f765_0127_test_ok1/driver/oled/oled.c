...
void OLED_WR_Byte2(u8 dat,u8 cmd)    //反白
{	
	u8 i;			  
	if(cmd)
	  OLED_DC_Set();
...
{  
	...
	{  
		...
		OLED_WR_Byte (0x00,OLED_CMD);      //设置显示位置—列低地址    OLED_WR_Byte (0x02,OLED_CMD);  
...
{      	
	unsigned char c=0,i=0;	
		...
		}else if(size == 12){	    ///-------------------第2种字体-----------------
				OLED_Set_Pos(x,y+1);
				//for(i=0;i<6;i++)
			for(i=0;i<8;i++)
				OLED_WR_Byte(F6x8[c][i],OLED_DATA);	
		}else if(size == 32){   //oled_audio/video  display  -------第3种字体-----------------
			OLED_Set_Pos(x,y);	
			for(i=0;i<16;i++)
...
extern void OLED_ShowChar_2(u8 x,u8 y,u8 chr,u8 size)   //反白
{      	
	unsigned char c=0,i=0;	
		c=chr-' ';//得到偏移后的值			
		if(x>Max_Column-1){x=0;y=y+2;}
...
{ 	 		 
  ...
  OLED_RST_Set();
	...
	if(Config_Display_Brightness_Arr[0] == MENU_SET)
		OLED_WR_Byte(0xff,OLED_CMD); // Set SEG Output Current Brightness          // 设置对比度  00 —— ff
	else
